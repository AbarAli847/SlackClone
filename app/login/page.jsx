  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4 font-sans text-[#1d1c1d] ">
      ...
      <div className="w-full max-w-[400px]">
        ...
        <button className="w-full bg-[#4a154b] text-white font-bold py-3 px-4 rounded-lg text-[18px] hover:bg-[#5d1c5e] transition-colors mb-3">
          Sign In With Email
        </button>
       
        <div className="flex items-center my-3">
          <div className="flex-grow border-t border-gray-200"></div>
          <span className="mx-4 text-sm font-semibold text-gray-500">OR SIGN IN WITH</span>
          <div className="flex-grow border-t border-gray-200"></div>
        </div>
 
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <button className="flex-1 flex items-center justify-center border-2 border-gray-300 py-3 rounded-lg font-bold hover:bg-gray-50 transition-colors">
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="h-5 w-5 mr-3" />
            Google
          </button>
          <button className="flex-1 flex items-center justify-center border-2 border-gray-300 py-3 rounded-lg font-bold hover:bg-gray-50 transition-colors">
            <svg className="h-5 w-5 mr-3" fill="currentColor" viewBox="0 0 24 24"><path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2.044-.156-3.33 1.04-4.303 1.04zM15.19 3.778c.857-1.04 1.442-2.48 1.287-3.91-1.221.052-2.704.819-3.587 1.846-.78.896-1.469 2.39-1.287 3.78 1.35.104 2.73-.676 3.587-1.716z"/></svg>
            Apple
          </button>
        </div>
      </div>  
      ...
      <div className="absolute top-8 right-8 text-sm">
        ...
        <a href="/register" className="text-blue-600 font-bold hover:underline">Create an account</a>
      </div>
